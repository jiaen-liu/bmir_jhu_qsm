...
if(exist(fullfile(cd, [outputFile, '.mat']), 'file') == 2 && Params.saveOutput)
    ...
    if ~single_step_flag
        ...
        if(exist(Params.FSLFolder, 'dir') == 7) || (ispc && ~isempty(cmdoutput))
            ...
            if ~isfield(Params,'FSLThreshold_g')
                Params.FSLThreshold_g=0.0;
            end
	    if ~isfield(handles.Params, 'FSLBETskip') % default, skip for non_brain data
		inputstring1 = [Params.FSLFolder, '/bet2 ', fname1, '.nii ' fname1, '_brain', ' -f ', num2str(Params.FSLThreshold), ' -g ',num2str(Params.FSLThreshold_g),' -m' ];
		...
		% for RAS NIFTI
            ...
            if length(Params.SaveEcho) > 1
                ...
                if ~isfield(handles.Params, 'FSLBETskip') % default, otherwise skip for non_brain data
                    inputstring1 = [Params.FSLFolder, '/bet2 ', fname2, '.nii ' fname2, '_brain', ' -f ', Params.FSLThreshold, ' -g ',num2str(Params.FSLThreshold_g),' -m' ];

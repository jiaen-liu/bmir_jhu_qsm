...
si=size(maskErode2);
if numel(si)>3
    nv=numel(maskErode2)/(si(1)*si(2)*si(3));
    maskErode2=reshape(maskErode2,[si(1:3),nv]);
    maskErode2=sum(maskErode2,4)==nv;
end
maskErode = maskErode.*imfill3(maskErode2);
freqMap = freqMap.*maskErode;
disp('Done.')
